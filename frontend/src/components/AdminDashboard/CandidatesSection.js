...
const CandidatesSection = () => {
  ...
  useEffect(() => {
    const fetchUnapprovedUsers = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_BACKEND_BASE_URL}/api/users/unapproved`);
  ...
  const approveUser = async (userId) => {
    try {
      await axios.put(`${process.env.REACT_APP_BACKEND_BASE_URL}/api/users/${userId}/approve`);
